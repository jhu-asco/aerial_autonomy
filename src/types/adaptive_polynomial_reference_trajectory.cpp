...
AdaptivePolynomialReferenceTrajectory::findBasisMatrix(double t, int degree,
  ...
  return wrapped_traj_.findBasisMatrix(t,degreee,dimensions);
}
...
PolynomialReferenceTrajectory::atTime(double t) const {
  ...
  Eigen::Matrix3d R;
  transformRPYToMatrix3d(roll,pitch,yaw,R);
...
ParticleStateYaw PolynomialReferenceTrajectory::goal(double) {
  Eigen::VectorXd goal_state = wrapped_traj_.goal();
  //Convert to particlestateyaw and snap;
