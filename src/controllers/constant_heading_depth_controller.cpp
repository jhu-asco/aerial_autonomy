...
VelocityYawRate ConstantHeadingDepthController::runImplementation(
    PositionYaw sensor_data, Position goal, ControllerStatus &) {
  ...
  tracking_error_ = (current_tracking_vector - desired_tracking_vector);
  tf::Vector3 tracking_error_radial =
      desired_tracking_direction *
      (tracking_error_.dot(desired_tracking_direction));
  tf::Vector3 tracking_error_tangential =
      tracking_error_ - tracking_error_radial;
  ...
  error_yaw_ = math::angleWrap(std::atan2(desired_tracking_direction.getY(),
                                          desired_tracking_direction.getX()) -
                               sensor_data.yaw);
  double yaw_rate =
      math::clamp(config_.yaw_gain() * error_yaw_, -config_.max_yaw_rate(),
                  config_.max_yaw_rate());
  return VelocityYawRate(desired_vel_tf.getX(), desired_vel_tf.getY(),
                         desired_vel_tf.getZ(), yaw_rate);
}

bool ConstantHeadingDepthController::isConvergedImplementation(PositionYaw,
                                                               Position) {
  ...
  // Compare
  if (std::abs(tracking_error_.x()) < tolerance_pos &&
      std::abs(tracking_error_.y()) < tolerance_pos &&
      std::abs(tracking_error_.z()) < tolerance_pos &&
      std::abs(error_yaw_) < tolerance_yaw) {
    VLOG(1) << "Reached goal";
    return true;
  }
  return false;
