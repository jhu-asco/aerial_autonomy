...
The GUI should have 1 text box per hardware plugin to get the status + health information of the hardware. It also needs a state text box for getting the current state and status updates towards progress of the goal.  The GUI should be able to send manual triggers to the state machine that are generated by the user. Here is an example GUI FrontEnd:
