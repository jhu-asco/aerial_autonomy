...
public:
  MPCControllerAirmConnectorTests()
    ...
    controller_.reset(new DDPAirmMPCController(controller_config_, 0.02));
  ...
  void runUntilConvergence(const PositionYaw &initial_state,
    ...
    ASSERT_FALSE(test_utils::waitUntilFalse()(
        runController, std::chrono::seconds(4), std::chrono::milliseconds(0)));
    ...
    ASSERT_VEC_NEAR(quad_transform.getOrigin(), goal_transform.getOrigin(),
                    controller_config_.goal_position_tolerance());
    tf::Vector3 zero_vec(0, 0, 0);
    ASSERT_VEC_NEAR(tf::Vector3(sensor_data.linvel.x, sensor_data.linvel.y,
